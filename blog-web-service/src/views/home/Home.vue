    ...
    <div class="home-banner" :style="cover">
      <div class="banner-container">
        ...
        <div class="blog-intro">
          {{ config.des }} <span class="typed-cursor"></span>
        </div>
        <!-- 联系方式 -->
      ...
      <v-col md="9" cols="12">
        <v-card>
          <div style="display: flex;margin-top: 20px;padding: 1rem 1rem">
            ...
            <span style="margin-left: 10px">{{ config.bulletin }}</span>
          </div>
        </v-card>
        <v-card
        ...
        <div class="blog-wrapper">
          <v-card class="animated zoomIn blog-card mt-5">
            ...
            <div class="card-info-social">
              ...
              />
              <a
                  v-if="config.isGiteeShow === 1"
                  target="_blank"
                  :href="config.giteeNumber"
                  class="iconfont icongitee-fill-round"
              />
            </div>
          </v-card>
          ...
          <v-card class="blog-card animated zoomIn mt-5 big">
            ...
            <div class="guanzhu" id="follow-us" ref="follow">
              {{ config.bulletin }}
              <ul>
                <li v-if="config.isQqShow === 1" class="qq">
                  <a
                      :href="
                ...
                <li v-if="config.isGiteeShow === 1" class="gitee">
                  <a :href="config.giteeNumber" target="_blank">{{
                    config.giteeNumber
                    }}</a>
                </li>
              </ul>
...
export default {
  ...
  methods: {
    ...
    fetchArticleList() {
      ...
      articleApi.queryArticleList(param).then(response => {
        if (response.code === 200) {
          this.articleList = response.data.respVos;
    ...
    // 查询文章数、分类数和标签数
    fetchBaseInfo() {
      articleApi.getBase().then(response => {
        if (response.code === 200) {
  ...
  computed: {
    ...
    cover() {
      // this.fetchWebsiteConfig();
      // var cover = this.config.indexBack;
      // todo 首页
      var cover = 'https://98k-hk666.oss-cn-shanghai.aliyuncs.com/iblog/article/20220730/5adab34b06124c38854fbe352334f1d9.jpg'
