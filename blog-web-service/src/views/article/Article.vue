        ...
        <v-card class="article-wrapper">
          ...
          <div class="aritcle-copyright">
            ...
            <div v-if="article.isOriginal === 1">
              <span>文章作者：{{ websiteConfig.author }}</span>
            </div>
          ...
          <div class="article-reward">
            ...
            <a :class="isLike" @click="like">
              ...
              点赞
              <span v-show="article.thumhup > 0">{{ article.thumhup }}</span>
            </a>
            <a class="reward-btn">
              <!-- 打赏按钮 -->
          ...
          <div class="pagination-post">
            <!-- 上一篇 -->
            <div
                :class="isFull(article.frontArticleId)"
                v-if="article.isFront === 1"
            >
              <router-link :to="'/articles/' + article.frontArticleId">
                <img class="post-cover" :src="article.frontArticleImg"/>
                <div class="post-info">
                  <div class="label">上一篇</div>
                  <div class="post-title">
                    {{ article.frontArticleTitle }}
                  </div>
                </div>
              </router-link>
            </div>
            <!-- 下一篇 -->
            <div
                :class="isFull(article.nextArticleId)"
                v-if="article.isNext === 1"
            >
              <router-link :to="'/articles/' + article.nextArticleId">
                <img class="post-cover" :src="article.nextArticleImg"/>
          ...
          <!-- 推荐文章 -->
          <div class="recommend-container" v-if="article.recommendArticleList">
...
import {fetchComments, like} from "../../api";
...
export default {
  ...
  methods: {
    // 根据文章id查询评论信息
    fetchComment() {
      articleApi.articleComment(this.articleId).then(response => {
    ...
    doSend(content) {
      if (!this.user) {
        ...
        if (content) {
          ...
          articleApi.saveComment(data).then(response => {
            if (response.code === 200) {
              this.fetchComment();
            }
          });
    ...
    // 获取最新文章
    fetchNewArticle() {
      articleApi.newArticle(this.articleId).then(response => {
        if (response.code === 200) {
          this.newestArticleList = response.data;
        }
      });
    },
    ...
    // 查询文章详情
    fetchArticleDetail() {
      articleApi.getById(this.articleId).then(response => {
        if (response.code === 200) {
    ...
    like() {
      ...
      // 发送请求
      const thumhup = this.$store.state.thumhup;
      if (thumhup === 1) {
        // 已经点过赞了 取消点赞
        articleApi.thumhup(-1, this.articleId).then(res => {
          if (res.code === 200) {
            this.article.thumhup--;
            this.$store.commit("saveThumhup", 0);
        ...
        articleApi.thumhup(1, this.articleId).then(res => {
          if (res.code === 200) {
            this.article.thumhup++;
            this.$store.commit("saveThumhup", 1);
